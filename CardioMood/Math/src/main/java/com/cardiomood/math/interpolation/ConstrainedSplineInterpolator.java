package com.cardiomood.math.interpolation;
...
public class ConstrainedSplineInterpolator implements UnivariateInterpolator {
    ...
    public static void main(String[] args) {
        PolynomialSplineFunction f = new ConstrainedSplineInterpolator()
                .interpolate(
                        new double[]{0, 10, 30, 50, 70, 90, 100},
                        new double[]{30, 130, 150, 150, 170, 220, 320}
                );
        double x = 0;
        while (x <= 100) {
            System.out.println(x + "\t" + f.value(x));
            x += 2;
        }
    }
}
