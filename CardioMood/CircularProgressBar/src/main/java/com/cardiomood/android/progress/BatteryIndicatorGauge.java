...
public class BatteryIndicatorGauge extends View {
    ...
    public void setValue(float value) {
        if (value < min || value > max) {
            throw new IllegalArgumentException("Illegal value (must be within [min, max])");
        }
        this.value = value;
        invalidate();
    }
...
//        canvas.drawPaint(mPaint);
        ...
        if (getOrientation() == HORIZONTAL) {
            contentRect.right = contentRect.left + contentRect.width() * getPercentage()/100;
        } else {
            contentRect.top = contentRect.top + contentRect.height() * (100 - getPercentage())/100;
        }
