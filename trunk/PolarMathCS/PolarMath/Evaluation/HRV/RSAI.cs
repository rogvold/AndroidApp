...
namespace PolarMath.Evaluation.HRV
{
    ...
    class RSAI : IEvaluation<double>
    {
        public double Evaluate(SessionData training)
        {
            ...
            var cv = training.Evaluate( new CV() );
            var mo = training.Evaluate( new Mo() );
            var amo = training.Evaluate( new AMoPercents() );
            var mxdmn = training.Evaluate( new MxDMn() );
            var si = training.Evaluate( new SI() );
            var lfPercents = training.Evaluate( new LFPercents() );
            var hfPercents = training.Evaluate( new HFPercents() );
            var vlfPercents = training.Evaluate( new VLFPercents() ) + training.Evaluate( new ULFPercents() );
